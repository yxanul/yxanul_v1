...
def train():
    ...
    for iter_num in range(config.max_iters):
        ...
        # Compute update/weight ratio stats per block right after step
        try:
            last_update_metrics = {}
        except NameError:
            last_update_metrics = {}
        ...
        try:
            def _layer_update_weight_stats(m: torch.nn.Module):
                block_rat = defaultdict(list)
                for name, p in m.named_parameters():
                    if not p.requires_grad:
                        continue
                    if not hasattr(p, '_prev_data'):
                        p._prev_data = p.data.detach().clone()
                        continue
                    if p.data.numel() == 0:
                        p._prev_data = p.data.detach().clone()
                        continue
                    upd = (p.data - p._prev_data)
                    p._prev_data = p.data.detach().clone()
                    upd_rms = (upd.norm(2) / math.sqrt(max(1, upd.numel()))).item()
                    w_rms = (p.data.norm(2) / math.sqrt(max(1, p.data.numel()))).item()
                    ratio = upd_rms / (w_rms + 1e-12)
                    bi = -1
                    parts = name.split('.')
                    for i, tok in enumerate(parts):
                        if tok == 'h' and i + 1 < len(parts):
                            try:
                                ...
                                bi = -1
                            break
                    block_rat[bi].append(ratio)
                blocks = sorted(k for k in block_rat.keys() if k >= 0)
                if not blocks:
                    return {}
                per_block = []
                for k in blocks:
                    vals = block_rat[k]
                    per_block.append(sum(vals) / max(1, len(vals)))
                per_block_sorted = sorted(per_block)
                first_r = per_block[0]
                last_r = per_block[-1]
                ratio_rlf = last_r / (first_r + 1e-12)
                med = per_block_sorted[len(per_block_sorted) // 2]
                return {
                    'upd/first_block_ratio': first_r,
                    'upd/last_block_ratio': last_r,
                    'upd/ratio_last_first': ratio_rlf,
                    'upd/min_block_ratio': per_block_sorted[0],
                    'upd/median_block_ratio': med,
                    'upd/max_block_ratio': per_block_sorted[-1],
                }
            last_update_metrics = _layer_update_weight_stats(model)
        except Exception:
            last_update_metrics = {}
        ...
        if iter_num % config.log_interval == 0:
            ...
            metrics = {
                ...
                'train/clip_rate': (clip_hits / max(1, total_steps)),
            }
            if args.opt == 'sophia':
                ...
                try:
                    ...
                    pass
            # Per-layer gradient RMS sampled every 100 steps
            if (iter_num % 100) == 0:
                try:
                    def _layer_grad_stats(m: torch.nn.Module):
                        block_rms = defaultdict(list)
                        for name, p in m.named_parameters():
                            if p.grad is None or p.grad.numel() == 0:
                                continue
                            g = p.grad.detach()
                            rms = (g.norm(2) / math.sqrt(max(1, g.numel()))).item()
                            bi = -1
                            parts = name.split('.')
                            for i, tok in enumerate(parts):
                                if tok == 'h' and i + 1 < len(parts):
                                    try:
                                        ...
                                        bi = -1
                                    break
                            block_rms[bi].append(rms)
                        blocks = sorted(k for k in block_rms.keys() if k >= 0)
                        if not blocks:
                            return {}
                        per_block = []
                        for k in blocks:
                            ...
                            per_block.append(sum(vals) / max(1, len(vals)))
                        per_block_sorted = sorted(per_block)
                        first_rms = per_block[0]
                        last_rms = per_block[-1]
                        ratio = last_rms / (first_rms + 1e-12)
                        med = per_block_sorted[len(per_block_sorted) // 2]
                        return {
                            'grad/first_block_rms': first_rms,
                            'grad/last_block_rms': last_rms,
                            'grad/ratio_last_first': ratio,
                            'grad/min_block_rms': per_block_sorted[0],
                            'grad/median_block_rms': med,
                            'grad/max_block_rms': per_block_sorted[-1],
                        }
                    metrics.update(_layer_grad_stats(model))
                except Exception:
                    pass
            # Include update/weight ratio metrics
            if last_update_metrics:
                metrics.update(last_update_metrics)
